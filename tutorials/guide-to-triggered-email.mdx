---
title: "A complete guide to setting up product triggered customer emails, sms, and push
"
description:
  "This guide will help you quickly set up a robust system where actions customers take in your product trigger customer messages."
---

### TLDR
Roughly speaking these are the steps required:
1) Install product analytics system (posthog) ~ 15mins
2) Specify user properties you want to use to personalize messages ~ 15mins
3) Use identify call in product analytics system correctly ~ 15mins
...
- The first step is to set up a product analytics system like mixpanel, amplitude, posthog, heap 
...
### Advantages to using twilio/sendgrid calls directly
Some of you might be wondering what the advantage of using these tools are to, say manually implementing a twilio call in your code when you want to send a message. 

Triggering messages with product analytics events + laudspeaker, makes it much easier to 
1) Implement complex journey logic, 
2) Modify/change messaging on the fly
3) Track messaging campaigns results
4) As a bonus laudspeaker is developer friendly

1) its difficult to implement drip campaigns with time delays, and complex branching logic with one-off api calls, a flow chart based editor is a better tool to visually understand what is happening.

2) Its very normal for people in the product/ marketing orgs to want to change, and modify customer messaging flows often to try and improve some kpi. Instead of having to rely on engineering for every modification, they can implement and makes changes on their own.

3) Systems like laudspeaker collate results, and correlate them to user outcomes. This functionalty comes out of the box

4) We are working towards creating a one to one mapping from our visual journeys and code (our journeys are already modeled as finite state machines), so that everything can be tested, versioned and tracked in a similar way to code if you prefer.

### What can you do with customer messaging and what are the results you can expect?

tldr: You can meaningfully move the needle on some core kpi with the right messaging nudges! 

If your product has a funnel for some core action, you can increase conversion between steps. 

One of our customers sends a simple drip campaign to users who abandon the funnel on the purchase step and in our estimates has seen a 10% uplift in sales in their self serve product.

You can also use Laudspeaker to do things that don't scale by being creative. One of our customers sends targeted emails with a calendly link and cc'es a team member when users drop off on a specific page. The team member is both notified of whom to target, and can proactively start a conversation if they want.
