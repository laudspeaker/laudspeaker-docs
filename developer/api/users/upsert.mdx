---
title: "Upsert"
description: "Adding or updating users programmatically via the Laudspeaker API"
api: 'POST https://api.laudspeaker.com/customers/upsert'

---

The `upsert` endpoint is designed to allow Luadspeaker users with the programmatic ability to add new customers or update existing customers.

The `upsert` endpoint looks up a customer document using the supplied `primary-key`, and then adds or changes customer attribute values specified in the `properties` `json` object in the request body.
...
### Body
<ParamField body="primary-key" type="string" required placeholder="email">
...
curl --request POST \
  ...
  --data '{
  "primary-key": "johnsmith@example.com"
}'
```
</CodeGroup>
